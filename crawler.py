...
def go():
	...
	SAVE_PAGES = save_pages.get()
	ZIP_FILES = zip_files.get()
	SAVE_WORDS = save_words.get()
	TODO_FILE = todo_file.get()
	DONE_FILE = done_file.get()
	BAD_FILE = bad_file.get()
	WORD_FILE = word_file.get()
	SAVE_COUNT = save_count.get()
	MAX_NEW_ERRORS = max_new_errors.get()
	MAX_HTTP_ERRORS = max_http_errors.get()
...
def setup_window():
	...
	global overwrite, raise_errors, save_pages, zip_files, save_words, todo_file, done_file, bad_file, word_file
	global custom_headers, save_count, max_new_errors, max_http_errors, max_known_errors, max_new_mimes
	...
	pause_button = ttk.Button(control_box, padding='4', text='Pause', command=pause)
	...
	go_button = ttk.Button(control_box, command=go, padding='4', text='Go')
